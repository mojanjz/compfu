...
class robot_controller:
    ...
    def callback(self,data):
        ...
        grayOutput = cv2.bitwise_and(warped_img, warped_img, mask = grayMask) #masking the cv_image
        ...
        print("cv_image")
        print(cv_image)
        grayPercentage =float( np.count_nonzero(np.asarray(grayOutput))) / float(np.count_nonzero(np.asarray(warped_img)))
